...
fn pidfile_proc(_spec: &arg::Spec<MyContext>, ctx: &mut MyContext,
    args: &Vec<String>) {
...
fn optarg() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let pidfile_spec = arg::Builder::new().sopt('p').lopt("pidfile")
      .help(&["Store process pid in FILE."])
      .nargs(arg::Nargs::Count(1), &["FILE"]).build(pidfile_proc);

  let ctx = MyContext{..Default::default()};

  let mut prsr = arg::Parser::from_args("optarg", &["--pidfile", "foobar.pid",
      "--file", "test.txt"], ctx);
